    ...
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base">{t('dashboard.workout.title')}</CardTitle>
          <Badge
            variant="outline"
            className={doneCount === 2 ? 'border-green-500/30 bg-green-500/10 text-green-600' : ''}
          >
            {t('dashboard.workout.done_count', { n: doneCount })}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="flex items-start gap-2 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2">
          ...
          <p className="text-xs text-amber-800 dark:text-amber-200 leading-relaxed">{t('dashboard.workout.weather_sunny')}</p>
        </div>
        ...
        {bothDone && (
          <div className="flex items-center gap-1.5 bg-green-500/10 border border-green-500/30 rounded-lg px-3 py-2">
            ...
            <span className="text-xs font-semibold text-green-700 dark:text-green-400">{t('dashboard.workout.both_complete')}</span>
          </div>
        )}
